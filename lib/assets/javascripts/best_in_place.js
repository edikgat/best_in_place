...
BestInPlaceEditor.prototype = {
    ...
    update: function () {
        ...
        if ((this.formType in {"input": 1, "textarea": 1} || this.formType == 'datetime') && value === this.oldValue) {
            this.abort();
            return true;
        }
        ...
        switch (this.formType) {
            case "select":
                ...
                $.each(this.values, function(index, arr){ if (String(arr[0]) === String(value)) editor.element.html(editor.escape(arr[1])); });
                break;
            ...
            default:
                if (value !== "") {
                    if (this.display_raw) {
                        editor.element.html(editor.escape(value));
                    } else {
